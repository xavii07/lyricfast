...
const Home = () => {
  const { ultimasbusquedas, setFavoritos, favoritos } =
    useContext(MusicContext);

  const agregarFavoritos = ({ id }) => {
    ultimasbusquedas.map((busqueda) =>
      busqueda.id === id ? setFavoritos([busqueda, ...favoritos]) : null
    );
...
const ContainerGrid = styled.div`
  ...
  grid-template-areas: "formu formu";
`;
